...
/* forward declaration of kernel game entry (implemented in kernel_game.c) */
...
/* NEW: kernel syscall handler to start the kernel game */
static int sys_game_start(int64_t *argptr)
{
    /* start_kernel_game runs in kernel context and blocks until it returns */
    start_kernel_game();
    return 0;
}
...
void init_system_call(void)
{
    ...
    system_calls[13] = sys_game_start;   /* register new syscall at index 13 */
}
...
void system_call(struct TrapFrame *tf)
{
    ...
    /* allow syscall index 0..13 (we added 13) */
    if (param_count < 0 || i < 0 || i > 13) { 
        tf->rax = -1;
        return;
    }
